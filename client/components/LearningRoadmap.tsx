...
import { Clock, BookOpen, Target, CheckCircle, ArrowRight, Star, Award, Users } from 'lucide-react';
...
export function LearningRoadmap({ 
  ...
  const handleLessonStudy = (moduleIndex: number, lessonIndex: number) => {
    ...
    const moduleStudiedCount = module.lessons.filter((_, idx) => 
      studiedLessons.has(`${moduleIndex}-${idx}`) || `${moduleIndex}-${idx}` === lessonKey
    ).length;
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-4">📚 Learning Roadmap</h1>
        ...
        <div className="bg-blue-50 rounded-lg p-6 mb-8">
          ...
          <div className="w-full bg-blue-200 rounded-full h-3">
            <div
              ...
              style={{ width: `${(completedModules.length / roadmap.length) * 100}%` }}
            />
          </div>
        </div>
      </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    <div className={cn(
                      ...
                      {isCompleted ? <CheckCircle className="w-6 h-6" /> : <span className="font-bold">{moduleIndex + 1}</span>}
                    </div>
                    <div className="text-left">
                      <h3 className="text-xl font-semibold text-gray-900">{module.module}</h3>
                      ...
                      return (
                        <div 
                          ...
                          className={cn(
                            ...
                            isStudied ? "border-green-300 bg-green-50" : "border-gray-200 hover:border-blue-300"
                          )}
                          onClick={() => handleLessonStudy(moduleIndex, lessonIndex)}
                        >
                          <div className="flex items-start justify-between mb-3">
                            <div className="flex-1">
                              <div className="flex items-center space-x-2 mb-2">
                                <h4 className="font-semibold text-gray-900">{lesson.title}</h4>
                                {isStudied && <CheckCircle className="w-5 h-5 text-green-600" />}
                              </div>
                              <p className="text-gray-600 text-sm mb-3">{lesson.summary}</p>
                            </div>
                          ...
                          {/* Learning Objectives */}
                          <div className="mb-3">
                            <h5 className="text-sm font-medium text-gray-800 mb-2">Learning Objectives:</h5>
                          ...
                          {/* Key Topics */}
                          <div>
                            <h5 className="text-sm font-medium text-gray-800 mb-2">Key Topics:</h5>
        ...
        <div className="text-center mt-8">
          <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg p-6 mb-6">
            <div className="flex items-center justify-center space-x-2 mb-2">
              ...
              <span className="text-lg font-semibold text-green-800">Roadmap Complete!</span>
            </div>
            <p className="text-green-700">
              Excellent progress! You've mastered all the foundational concepts. Ready for visual learning?
            </p>
          </div>
          <button
            onClick={onContinue}
