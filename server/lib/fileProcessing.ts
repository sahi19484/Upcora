...
async function getPdfParser() {
  try {
    ...
    throw new Error(
      "PDF library not available. Please try converting your PDF to text format.",
    );
  }
}
...
async function getMammoth() {
  try {
    ...
    throw new Error(
      "Word document processing library not available. Please try uploading a text file.",
    );
  }
}
...
async function getJSZip() {
  try {
    ...
    throw new Error(
      "PowerPoint processing library not available. Please try uploading a text file.",
    );
  }
}
  ...
  try {
    ...
    for (const masterFile of masterFiles) {
      try {
        ...
        if (masterXml) {
          const masterTextMatches =
            masterXml.match(/<a:t[^>]*>([^<]+)<\/a:t>/g) || [];
          const masterText = masterTextMatches
            .map((match) => match.replace(/<[^>]+>/g, "").trim())
            .filter(
              (text) =>
                text.length > 0 &&
                !text.match(/^(Click to edit|Add your|Sample)/i),
            )
            .join(" ");
    ...
    switch (mimeType) {
      case "application/pdf":
        try {
          ...
          if (!text.trim()) {
            throw new Error(
              "No text content found in PDF. The PDF may be image-based or protected. Please try saving as a text file.",
            );
          }
        } catch (pdfError) {
          ...
          if (
            pdfError.message.includes("ENOENT") ||
            pdfError.message.includes("test/data")
          ) {
            throw new Error(
              "PDF processing library not properly installed. Please try uploading a text file instead.",
            );
          } else if (pdfError.message.includes("Invalid PDF")) {
            throw new Error(
              "Invalid or corrupted PDF file. Please try re-saving the PDF or converting to text format.",
            );
            ...
            throw new Error(
              `Failed to process PDF: ${pdfError.message}. Please try saving as a text file or ensure the PDF contains readable text.`,
            );
          }
        }
        break;
      ...
      case "application/msword":
        try {
          ...
          throw new Error(
            `Failed to process Word document: ${wordError.message}. Please try saving as a text file.`,
          );
        }
        break;
      ...
      case "application/x-mspowerpoint":
        try {
          ...
          if (
            ...
            throw new Error(
              "Legacy PPT format not supported. Please save your PowerPoint as PPTX format for text extraction.",
            );
          }
        } catch (pptError) {
          if (pptError instanceof Error) {
            if (pptError.message.includes("No text content found")) {
              throw new Error(
                "No readable text found in PowerPoint file. Please ensure your slides contain text content, or try saving each slide as a text file.",
              );
            } else if (pptError.message.includes("Legacy PPT format")) {
              throw pptError;
            } else {
              throw new Error(
                `Failed to process PowerPoint: ${pptError.message}. Please try saving slides as a text file or ensure the file is a valid PPTX format.`,
              );
            }
          } else {
            throw new Error(
              "Failed to process PowerPoint file. Please try saving slides as a text file or convert to PPTX format.",
            );
          }
        }
        break;
      ...
      default:
        ...
        try {
          ...
          if (
            ...
            throw new Error(
              `Unsupported file type: ${mimeType}. Please upload PDF, Word, PowerPoint, or text files.`,
            );
          }
        } catch {
          throw new Error(
...
export async function extractTextFromUrl(
  ...
  try {
    ...
    if (!text || text.length < 100) {
      throw new Error(
        "URL content appears to be empty or too short for processing",
      );
    }
